...
def spark_batch():
    ...
    df = spark.read \
        .format("parquet") \
        .schema(schema) \
        .load(f"abfss://trips-info-container-landing@{os.getenv('STORAGE_ACCOUNT_NAME')}.dfs.core.windows.net/data/")
    ...
    enriched_df.write \
        ...
        .mode("append") \
        .save(f"abfss://trips-info-container-staging@{os.getenv('STORAGE_ACCOUNT_NAME')}.dfs.core.windows.net/data/")

    print(f"Batch completado: {enriched_df.count()} registros procesados de landing a staging.")
    spark.stop()
